...
const baseQuery = fetchBaseQuery({
  ...
  prepareHeaders: (headers) => {
    const token = Cookies.get("main.bonavent_accessToken");
    const signUpToken = getFromLocalStorage("main.bonavent_createUserToken");

    const changePassToken = Cookies.get("main.bonavent_forgetToken");
    const forgetOtpMatchToken = Cookies.get(
      "main.bonavent_forgetOtpMatchToken",
    );
